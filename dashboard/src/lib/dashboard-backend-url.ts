 ...
 *
 * On **Railway**, prefer the **private** URL
 * `http://${{dashboard-backend.RAILWAY_PRIVATE_DOMAIN}}:${{dashboard-backend.PORT}}`
 * (set by `setup-railway.js` when you run `SYNC_VARIABLES=1`). Server-side `fetch` to
 * the public HTTPS URL can fail (routing/DNS/empty domain); private networking matches
 * how the load balancer reaches the API replicas.
 *
