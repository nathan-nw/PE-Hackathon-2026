...
export async function GET(request: NextRequest) {
  ...
  const url = `${resolved.base}/api/logs/insights${qs ? `?${qs}` : ""}`;
  ...
  try {
    ...
    if (onRailway && resolved.usedDefaultLocalhost) {
      ...
      hint =
        "If the error is ENOTFOUND or connection refused, confirm the **dashboard** service can reach **dashboard-backend** on the private network.";
    }
    return NextResponse.json(
      {
        buckets: [],
