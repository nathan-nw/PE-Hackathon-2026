...
Operations visibility UI: **Docker** (Compose project filter) or **Railway** (GraphQL — no Docker socket in the cloud), Alertmanager alerts, **Kafka-backed app logs** (via `dashboard-backend`), and links to Prometheus and Alertmanager.
...
Open [http://localhost:3000](http://localhost:3000). For Docker-backed visibility you need the Docker socket on the host (see below).
